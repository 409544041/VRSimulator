...
public class TimerControllerLiver : MonoBehaviour
{
    ...
    void Timer()
    {
        ...
        {            
            if(extraTime == 0)//if liver wasnt throw into the bin then there is no extra time so the game ends after 3mins
            {
                GameOver();
                GameObject.Find("SeizureText").transform.localScale = new Vector3(0, 0, 0);
            }            
        ...
        {
            if (extraTime == 1 ) //liver was thrown into the bin so the game ends with extra 20 seconds
            {
    ...
    void GameOver()
    {
        ...
        GameObject.Find("LiverHPWithXR").transform.GetComponent<LiverGravity>().enabled = false;
        ...
        GameObject.FindWithTag("LiverCube").transform.localPosition = new Vector3(0, 0, 0);
